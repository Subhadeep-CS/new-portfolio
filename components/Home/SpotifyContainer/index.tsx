...
const SpotifyContainer = () => {
    ...
    return (
        <Inspectable
            metadata={{
                name: "SpotifyContainer.tsx",
                description: "Dynamic widget displaying real-time 'Now Playing' status from Spotify using the Web API and Next.js Route Handlers.",
                stack: ["Spotify Web API", "Next.js Route Handlers", "Framer Motion", "React Hooks"],
                optimizations: [
                    "SWR-style periodic polling (30s interval)",
                    "Conditional animation rendering (only when playing)",
                    "Dynamic album art loading"
                ],
                patterns: ["API Polling Pattern", "Hydration Guard", "Visual Soundwave Simulation"],
                architectureNotes: "Uses a dedicated backend route (/api/spotify) to securely handle OAuth tokens, preventing sensitive credentials from leaking to the client.",
                animation: {
                    type: "Infinite Waveform (Framer Motion)",
                    duration: "1.2s loop",
                    description: "Staggered height animation for 5 bars to simulate an equalizer."
                },
                accessibility: ["ARIA live regions (hidden)", "Semantic HTML", "Interactive focus states"],
                buildProcess: [
                    { iteration: "v1", note: "Static UI with placeholder data." },
                    { iteration: "v2", note: "Integrated live API with Next.js API routes." },
                    { iteration: "v3", note: "Added motion-driven equalizer and hover effects." }
                ]
            }}
        >
